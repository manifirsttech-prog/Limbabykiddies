        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="bg-white rounded-2xl shadow-xl border-2 border-pink-100 p-8">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input id="admin-email" type="email" required value={email} onChange={(e) => { setEmail(e.target.value); setError(''); }} className="w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-300 focus:border-pink-300 text-sm transition-all" placeholder="admin@littlebloom.com" />
            </div>
            <div>
          ...
          <p className="text-xs text-gray-500 text-center mt-4">Demo: admin@littlebloom.com / admin123</p>
        </motion.div>
      </motion.div>
    </main>
  );
